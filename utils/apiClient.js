...
class AIClient {
  ...
  async callQwen({ prompt, imgUrl }) {
    ...
    try {
        ...
        messages: [
          {
            ...
            content: [
              ...
              {
                type: "text",
                text: "请仔细分析这张图片，识别图片中的物体并推算每件物体的保质期。请严格按照以下数据格式返回结果，不要添加任何额外的文本或解释：[{\"name\": \"物体名称\", \"date\": \"保质期\"}]。保质期请以天数形式表示，例如'7天'、'30天'等。",
              },
            ],
          },
        ],
      ...
      console.log(
        "API密钥状态:",
        this.config.qwen.apiKey ? "已设置" : "未设置"
      );
  ...
  async withRetry(apiCall, maxRetries = 3) {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        lastError = error;
        console.log(
          `尝试 ${attempt + 1} 失败，${maxRetries - attempt - 1} 次重试机会`
        );
